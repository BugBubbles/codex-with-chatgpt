...
3. `execution.write` is required for `python_write_file` and `python_execute`.
...
Tokens, SSH agent sockets, cloud credentials, proxy credentials and arbitrary bridge environment variables are not inherited.
...
- The read-only Python runtime allow-list may expose public system library/package data.
